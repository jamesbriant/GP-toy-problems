...
# This version is the same as Model.py, except
# the data vector has been inverted.
# Model.py:  d=(y,z)
# Model2.py: d=(z,y)
...
class Model:
    ...
    def H_eval(self):
        ...
        # self.H[:self.y_n, :1] = self.H1D1
        # self.H[self.y_n:, :1] = self.params['rho']*self.H1D2
        # self.H[self.y_n:, :2] = self.H2D2

        self.H[:self.z_n, :1] = self.params['rho']*self.H1D2
        self.H[:self.z_n, :2] = self.H2D2
        self.H[self.z_n:, :1] = self.H1D1

    
    def m_d_eval(self):
        # self.m_d = np.dot(self.H, np.hstack([self.params['beta1'], self.params['beta2']]))
        self.m_d = np.dot(self.H, np.hstack([self.params['beta2'], self.params['beta1']]))
    ...
    def V_d_eval(self):
        ...
        # self.V_d[:a, :a] = self.V1D1
        # self.V_d[a:, :a] = self.params['rho']*self.C1D1D2
        # self.V_d[:a, a:] = np.transpose(self.V_d[a:, 0:a])
        # self.V_d[a:, a:] = self.V2D2 + self.V1D2*self.params['rho']**2 + self.hyperparams['lambda']*np.identity(b)
        
        self.V_d[b:, b:] = self.V1D1
        self.V_d[:b, b:] = self.params['rho']*self.C1D1D2
        self.V_d[b:, :b] = np.transpose(self.V_d[:b, b:])
        self.V_d[:b, :b] = self.V2D2 + self.V1D2*self.params['rho']**2 + self.hyperparams['lambda']*np.identity(b)
